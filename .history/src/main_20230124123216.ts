...
import { View101, VIEW_TYPE_101 } from './view通用编辑器视图';
...
function modal_输入弹窗(app实例: App, 输入标题:string, 
    ...
    new 输入弹窗(app实例, 点击提交按钮, 输入标题, 输入1描述, 输入2描述, 输入3描述
        ).open();
    return 返回值
}
...
export default class Vue101tool extends Plugin {
    ...
    async onload() { //* 在用户激活插件和插件更新时触发, 插件初始化基础功能的主要地方
        ...
        this.addCommand({ //* 打开可输入对话框
            ...
            editorCallback: (editor:Editor) => {
                const 点击提交按钮 = (array: Array<any>) => {
                    ...
                    let 输入2=array[1]
                    let 输入3=array[2]
                    editor.replaceRange(输入1, editor.getCursor())
                    editor.replaceRange(输入2, editor.getCursor())
                    editor.replaceRange(输入3, editor.getCursor())
                };
            
                new 输入弹窗(this.app, 点击提交按钮, "输入标题", "输入变量1", "输入变量2", "输入变量3",
                    ).open();
            },
        ...
        // 注册自定义的编辑器视图
        this.registerView(VIEW_TYPE_101, (leaf: WorkspaceLeaf)=> new View101(leaf))
        this.registerExtensions(["py"], VIEW_TYPE_101);
        this.registerExtensions(["bat"], VIEW_TYPE_101);
        this.registerExtensions(["csv"], VIEW_TYPE_101);
        this.registerExtensions(["c"], VIEW_TYPE_101);
        this.registerExtensions(["h"], VIEW_TYPE_101);
        this.registerExtensions(["java"], VIEW_TYPE_101);
        this.registerExtensions(["txt"], VIEW_TYPE_101);
