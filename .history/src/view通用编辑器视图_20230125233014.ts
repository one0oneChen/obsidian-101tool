...
export class View101_通用 extends TextFileView {
    ...
    setViewData(data: string, clear: boolean) { //* 设置编辑器视图如何显示 */
        ...
        const vueapp = createApp(codemirror) // 创建按钮vue实例
        vueapp.provide('cm配置', {}) //将对话款配置参数下传
        ...
        // const vueapp=挂载vue组件( codemirror, this.containerEl)
        // vueapp.
        // let 保存文件刷新视图 = ()=>{
        //     this.data=this.textareaEl.value
        //     console.log(`这里是保存文件刷新视图:${this.textareaEl.value}`)
        //     this.save();
        // }
        // this.textareaEl=this.containerEl
        //     // .createEl("div",) // 用div标签来包裹要显示的文本内容
        //     .createEl("textarea", { attr:{ style:"height:800px; width:100%;", } }) //
        
        //     this.textareaEl.value=this.data
