...
<script lang="ts" setup>
    ...
    let theme=oneDark  //语言及语法模式
    
    let 当前语言=app注入数据.使用语言
    // let extensions:any=ref([])
    switch (当前语言){
        case 'python':{var extensions=[python(),oneDark];}
        case 'c':{var extensions=[cpp(),oneDark];}
        case 'c++':{var extensions=[cpp(),oneDark];}
        case 'java':{var extensions=[java(),oneDark];}
        case 'javascript':{var extensions=[javascript(),oneDark];}
    ...
    function 处理下拉列表选择(key: string | number){
        ...
        app注入数据.mitt触发器.emit("emit监听语言切换", key)
