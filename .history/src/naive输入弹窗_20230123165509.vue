    ...
    <div class="vue_div">
        <ol>
            <li v-for="提示 in 输入提示列表" :key="提示.id">
                <p class="标题">{{ 提示.标题 }} #{{提示.id}}</p>
                <span class="描述">{{提示.描述}}</span> 
                <input type="text" @input="获取输入值($event, 提示.标题)" :placeholder="提示.默认文本" />
            </li>
        </ol>
...
<script lang="ts">
    ...
    export default {
        ...
        methods:{
            获取输入值(event:any, 标题: string){
                this.输入值列表.push(event.currentTarget.value as never);
                console.log(`遍历后输入值列表:${this.输入值列表}`);
            }
        }   
    }
</script>
