...
export class View101_通用 extends TextFileView {
    ...
    setViewData(data: string, clear: boolean) { //* 设置编辑器视图如何显示 */
        ...
        emitter.on("emit监听文件修改", (value:any)=>{
            this.data =value
            console.log(`emitter文件修改`)
            this.save()
        })
        
        emitter.on("emit监听语言主题切换", (value:any)=>{
            console.log(`emit监听语言主题切换`)
            vueapp.provide('app注入数据', { mitt触发器:emitter, 
                文件初始文本:this.data, 使用语言:value[0], 主题:value[1]}) //将对话款配置参数下传
            vueapp.unmount() // 从容器卸载
            vueapp.mount(this.containerEl) //重新挂载
        })

        let vueapp = createApp(codemirror) // 创建按钮vue实例
        vueapp.provide('app注入数据', { mitt触发器:emitter, 
            文件初始文本:this.data, 使用语言:"python", 主题:"dark" }) //将对话款配置参数下传
