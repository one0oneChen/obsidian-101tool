...
import naive输入弹窗 from "./naive弹窗.vue" //导入vue组件为naive按钮
...
export class vue输入弹窗 extends Modal {
    ...
    constructor(app: App, 输入1:string, 输入2:string, 输入3:string, ) {
        super(app);
        this.输入1=输入1
        this.输入2=输入2
        this.输入3=输入3
    ...
    onOpen() {
        ...
        containerEl.empty()
        const vue输入弹窗容器=containerEl.createEl('div', { cls: "vue_div"});
        const naive输入弹窗实例 = createApp(naive输入弹窗) // 创建按钮vue实例
        naive输入弹窗实例.mount(vue输入弹窗容器) // 将vue按钮实例挂载到容器元素
    }
