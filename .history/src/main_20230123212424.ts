...
function modal_输入1弹窗(app实例: App, 输入1标题:string, 输入1描述:string):string {
    let 返回值=""
    new 输入弹窗(
        app实例, 
        (result) => {
            // new Notice(`输入的是: ${result}!`);
            返回值=result;
        }, 
        输入1标题,输入1描述,
    ).open();
...
function modal_vue输入1弹窗(app实例: App, 
        // 输入1标题:string, 输入1描述:string
        ) {
    let 返回值=""
    let vueapp=new vue输入弹窗(
        app实例, [
...
export default class Vue101tool extends Plugin {
    ...
    async onload() { //* 在用户激活插件和插件更新时触发, 插件初始化基础功能的主要地方
        ...
        this.addCommand({ //* 打开可输入对话框
            ...
            name: '101tool_输入弹窗',
            callback: () => {
                let 返回值=modal_输入1弹窗(this.app, "输入测试", "这是描述");
                // modal_消息弹窗(this.app, 返回值);
                print(`输入的值是:${返回值}`)
                // this.editor.setValue(返回值);
                // this.editor.replaceSelection(返回值, "");
            }})
        ...
        this.addCommand({ //* 打开可输入对话框
            ...
            callback: () => {
                let 返回值=modal_vue输入1弹窗(this.app, );
                print(`输入的值是:${返回值}`)
            }})
...
// export { print, modal_消息弹窗, modal_输入1弹窗}