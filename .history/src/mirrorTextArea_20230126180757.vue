<template>
    ...
    <span class="nbutton">
        <n-dropdown trigger="hover" :options="语言下拉列表项目" ref="语言下拉"
            @select="处理下拉列表选择">
            <n-button >当前语言:{{当前语言}}</n-button>
        </n-dropdown>
    </span>
    <span class="nbutton">
        <n-dropdown trigger="hover" :options="主题下拉列表项目" ref="主题下拉"
            @select="处理下拉列表选择">
            <n-button >主题:{{theme}}</n-button>
        </n-dropdown>
    </span>
    <!-- <div>
    </div> -->
...
<script lang="ts" setup>
    ...
    function 处理下拉列表选择(){
        ...
        app注入数据.mitt触发器.emit("emit监听语言切换", [key, 主题])
        return 
    }
